...
class User(AbstractUser):
    ...
    blocked_until = models.DateTimeField(
        null=True,
        blank=True,
        help_text="Date until which the user is blocked"
    )
    ...
    def clean(self):
        ...
        if self.role == 'CUSTOMER' and not self.customer:
            raise ValidationError('Los usuarios con rol CLIENTE deben tener un cliente asociado')
        
        if self.role in ['OPERATOR', 'ADMIN'] and self.customer:
            raise ValidationError('Los usuarios OPERADOR/ADMIN no pueden tener cliente asociado')
