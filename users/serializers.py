...
class UserSerializer(serializers.ModelSerializer):
    ...
    is_blocked = serializers.BooleanField(
        read_only=True, help_text="Whether the user is temporarily blocked"
    )
    customer_name = serializers.CharField(
        source="customer.full_name",
        read_only=True,
        help_text="Associated customer name",
    )
    ...
    def validate(self, attrs):
        ...
        if attrs["role"] == "CUSTOMER" and not attrs.get("customer"):
            raise serializers.ValidationError(
                "Customer role requires an associated customer"
            )

        if attrs["role"] in ["OPERATOR", "ADMIN"] and attrs.get("customer"):
            raise serializers.ValidationError(
                "Operator/Admin roles cannot have associated customer"
            )
...
class UserUpdateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ["email", "first_names", "last_names", "is_active"]
        extra_kwargs = {
