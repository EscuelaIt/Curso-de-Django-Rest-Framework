...
class Customer(models.Model):
    ...
    document_number = models.CharField(
        max_length=20,
        validators=[
            RegexValidator(r"^[A-Z0-9]+$", "Only uppercase letters and numbers")
        ],
    ...
    updated_at = models.DateTimeField(auto_now=True, help_text="Last data update")
        ...
        if self.email:
            if not self.email.lower().endswith("@escuela.it"):
                raise ValidationError(
                    "Solo se permiten correos del dominio @escuela.it"
                )
