...
class CustomerAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Identificación", {"fields": ("document_type", "document_number")}),
        (
    ...
    def age_display(self, obj):
        ...
        if age:
            if age < 18:
                return format_html(
                    '<span style="color: red;">{} años (Menor)</span>', age
                )
            elif age >= 65:
                return format_html(
                    '<span style="color: blue;">{} años (Senior)</span>', age
                )
            else:
                return f"{age} años"
        return "-"
    ...
    def activate_customers(self, request, queryset):
        ...
        self.message_user(request, f"{count} cliente(s) activado(s) exitosamente.")
    ...
    def suspend_customers(self, request, queryset):
        ...
        self.message_user(request, f"{count} cliente(s) suspendido(s) exitosamente.")
    ...
    def deactivate_customers(self, request, queryset):
        ...
        self.message_user(request, f"{count} cliente(s) inactivado(s) exitosamente.")
