...
class CustomerSerializer(serializers.ModelSerializer):
    ...
    full_document = serializers.CharField(
        read_only=True, help_text="Document type and number"
    )
    accounts_count = serializers.SerializerMethodField(
        help_text="Number of savings accounts"
    )
    ...
    def validate(self, attrs):
        ...
        if document_type and document_number:
            if document_type == "DNI" and len(document_number) != 8:
                ...
                raise serializers.ValidationError(
                    "Carnet de Extranjería must have exactly 9 characters"
                )
...
class CustomerCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        extra_kwargs = {
            ...
            "document_number": {
                "required": True,
                "help_text": "Document number (8 digits for DNI, 9 for CE)",
            },
    ...
    def validate_email(self, value):
        ...
        if not value.lower().endswith("@escuela.it"):
            raise serializers.ValidationError(
                "Only @escuela.it email addresses are allowed"
            )
...
class CustomerUpdateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Customer
        fields = ["first_names", "last_names", "email", "phone", "address"]
        extra_kwargs = {
...
class CustomerStatusSerializer(serializers.Serializer):
    ...
    status = serializers.ChoiceField(
        choices=STATUS_CHOICES, help_text="New status for the customer"
    )
...
class CustomerSummarySerializer(serializers.ModelSerializer):
    ...
    def get_recent_transactions(self, obj):
        ...
        return Transaction.objects.filter(
            source_account__customer=obj, transaction_date__gte=thirty_days_ago
        ).count()
