...
class AuditSerializer(serializers.ModelSerializer):
    ...
    operation_display = serializers.CharField(
        read_only=True, help_text="Human-readable operation name"
    )
    has_data_changes = serializers.BooleanField(
        read_only=True, help_text="Whether data changes are recorded"
    )
    changed_fields = serializers.ListField(
        read_only=True, help_text="List of fields that were changed"
    )
    changes_summary = serializers.CharField(
        source="get_changes_summary", read_only=True
    )
...
class AuditFilterSerializer(serializers.Serializer):
    ...
    affected_table = serializers.CharField(
        required=False, help_text="Filter by affected database table"
    )
    record_id = serializers.IntegerField(
        required=False, help_text="Filter by specific record ID"
    )
    ...
    user_id = serializers.IntegerField(
        required=False, help_text="Filter by user who performed the operation"
    )
    date_from = serializers.DateTimeField(
        required=False, help_text="Filter operations from this date (inclusive)"
    )
    date_to = serializers.DateTimeField(
        required=False, help_text="Filter operations until this date (inclusive)"
    )
    context = serializers.CharField(
        required=False, help_text="Filter by operation context"
    )
    ip_address = serializers.IPAddressField(
        required=False, help_text="Filter by IP address"
    )
...
class AuditSummarySerializer(serializers.Serializer):
    ...
    total_operations = serializers.IntegerField(
        read_only=True, help_text="Total number of operations"
    )
    operations_by_type = serializers.DictField(
        read_only=True, help_text="Count of operations by type"
    )
    operations_by_table = serializers.DictField(
        read_only=True, help_text="Count of operations by table"
    )
    operations_by_user = serializers.DictField(
        read_only=True, help_text="Count of operations by user"
    )
    recent_activity = serializers.ListField(
        read_only=True, help_text="Recent audit entries"
    )
    date_range = serializers.DictField(
        read_only=True, help_text="Date range of the summary"
    )
...
class AuditTableSummarySerializer(serializers.Serializer):
    ...
    total_operations = serializers.IntegerField(
        read_only=True, help_text="Total operations on this table"
    )
    operations_by_type = serializers.DictField(
        read_only=True, help_text="Operations breakdown by type"
    )
    most_active_users = serializers.ListField(
        read_only=True, help_text="Users with most operations on this table"
    )
    recent_changes = serializers.ListField(
        read_only=True, help_text="Recent changes to this table"
    )
    affected_records = serializers.IntegerField(
        read_only=True, help_text="Number of unique records affected"
    )
...
class AuditUserActivitySerializer(serializers.Serializer):
    ...
    total_operations = serializers.IntegerField(
        read_only=True, help_text="Total operations by this user"
    )
    operations_by_type = serializers.DictField(
        read_only=True, help_text="Operations breakdown by type"
    )
    operations_by_table = serializers.DictField(
        read_only=True, help_text="Operations breakdown by table"
    )
    recent_activity = serializers.ListField(
        read_only=True, help_text="Recent operations by this user"
    )
    first_activity = serializers.DateTimeField(
        read_only=True, help_text="Date of first recorded activity"
    )
    last_activity = serializers.DateTimeField(
        read_only=True, help_text="Date of most recent activity"
    )
...
class AuditRecordHistorySerializer(serializers.Serializer):
    ...
    total_changes = serializers.IntegerField(
        read_only=True, help_text="Total number of changes"
    )
    creation_date = serializers.DateTimeField(
        read_only=True, help_text="When the record was created"
    )
    last_modified = serializers.DateTimeField(
        read_only=True, help_text="When the record was last modified"
    )
    change_history = serializers.ListField(
        read_only=True, help_text="Chronological list of all changes"
    )
    users_involved = serializers.ListField(
        read_only=True, help_text="Users who have modified this record"
    )
    current_status = serializers.CharField(
        read_only=True, help_text="Current status of the record"
    )
...
class AuditExportSerializer(serializers.Serializer):
    ...
    format = serializers.ChoiceField(
        choices=FORMAT_CHOICES, default="json", help_text="Export format"
    )
    date_from = serializers.DateTimeField(
        required=True, help_text="Start date for export (required)"
    )
    date_to = serializers.DateTimeField(
        required=True, help_text="End date for export (required)"
    )
    affected_table = serializers.CharField(
        required=False, help_text="Filter by specific table (optional)"
    )
    operation = serializers.ChoiceField(
        choices=Audit.OPERATION_CHOICES,
        required=False,
    ...
    user_id = serializers.IntegerField(
        required=False, help_text="Filter by specific user (optional)"
    )
    include_data_changes = serializers.BooleanField(
        default=True, help_text="Include detailed data changes in export"
    )
