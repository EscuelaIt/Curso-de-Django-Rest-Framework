...
class Transaction(models.Model):
    ...
    reference_number = models.CharField(
        max_length=50, unique=True, help_text="Unique transaction reference number"
    )
    transaction_type = models.CharField(
        max_length=30, choices=TYPE_CHOICES, help_text="Type of transaction"
    )
    ...
    description = models.CharField(
        max_length=500, blank=True, help_text="Transaction description"
    )

    # Balance control fields
    previous_balance = models.DecimalField(
        max_digits=15, decimal_places=2, help_text="Balance before transaction"
    )
    new_balance = models.DecimalField(
        max_digits=15, decimal_places=2, help_text="Balance after transaction"
    )
    ...
    processing_date = models.DateTimeField(
        null=True, blank=True, help_text="Transaction processing date"
    )
    ...
    ip_address = models.GenericIPAddressField(help_text="IP address of origin")
    user_agent = models.CharField(
        max_length=500, blank=True, help_text="User agent information"
    )
    ...
    def clean(self):
        ...
        if (
            self.transaction_type in ["TRANSFER_OUT", "TRANSFER_IN"]
        ...
        if (
            ...
            raise ValidationError(
                "Deposits and withdrawals should not have destination account"
            )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.previous_balance is None:
            self.previous_balance = (
                self.source_account.current_balance if self.source_account else 0
            )
