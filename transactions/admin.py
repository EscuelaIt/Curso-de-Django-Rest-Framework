...
class TransactionAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (
            ...
            {"fields": ("reference_number", "transaction_type", "description")},
        ),
        ("Cuentas Involucradas", {"fields": ("source_account", "destination_account")}),
        (
            "Detalles Financieros",
            {"fields": ("amount", "currency", "previous_balance", "new_balance")},
        ),
        (
            "Estado y Fechas",
            {"fields": ("status", "transaction_date", "processing_date")},
        ),
        (
            "Auditoría",
    ...
    def process_transactions(self, request, queryset):
        ...
        self.message_user(
            request, f"{count} transacción(es) procesada(s) exitosamente."
        )
    ...
    def reject_transactions(self, request, queryset):
        ...
        self.message_user(
            request, f"{count} transacción(es) rechazada(s) exitosamente."
        )
