...
class SavingsAccountSerializer(serializers.ModelSerializer):
    ...
    customer_document = serializers.CharField(source='customer.full_document', read_only=True)
    balance_display = serializers.CharField(read_only=True, help_text="Formatted balance with currency")
    is_active = serializers.BooleanField(read_only=True, help_text="Whether account is active")
    is_blocked = serializers.BooleanField(read_only=True, help_text="Whether account is blocked")
    transactions_count = serializers.SerializerMethodField(help_text="Total number of transactions")
    
    class Meta:
        model = SavingsAccount
        fields = [
            'id', 'account_number', 'customer', 'customer_name', 'customer_document',
            'current_balance', 'available_balance', 'currency', 'balance_display',
            'opening_date', 'last_movement_date', 'status', 'version',
            'is_active', 'is_blocked', 'transactions_count'
        ]
        read_only_fields = [
            'id', 'account_number', 'opening_date', 'last_movement_date', 'version',
            'customer_name', 'customer_document', 'balance_display', 'is_active',
            'is_blocked', 'transactions_count'
        ]
        extra_kwargs = {
            'customer': {'required': True, 'help_text': 'Account owner customer ID'},
            'currency': {'required': True, 'help_text': 'Account currency (PEN, USD, EUR)'},
        }
    ...
    def validate(self, attrs):
        ...
        current_balance = attrs.get('current_balance', getattr(self.instance, 'current_balance', Decimal('0.00')))
        available_balance = attrs.get('available_balance', getattr(self.instance, 'available_balance', Decimal('0.00')))
        
        if available_balance > current_balance:
            raise serializers.ValidationError("Available balance cannot be greater than current balance")
...
class SavingsAccountCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = SavingsAccount
        fields = [
            'customer', 'currency', 'initial_deposit'
        ]
        extra_kwargs = {
            'customer': {'required': True, 'help_text': 'Customer who will own the account'},
            'currency': {'required': True, 'help_text': 'Account currency (PEN, USD, EUR)'},
        }
...
class SavingsAccountListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = SavingsAccount
        fields = [
            'id', 'account_number', 'customer_name', 'current_balance',
            'currency', 'balance_display', 'status', 'status_display',
            'opening_date', 'last_movement_date', 'days_since_opening'
        ]
...
class SavingsAccountBalanceSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'currency', 'balance_display', 'last_movement_date'
        ]
...
class SavingsAccountStatusSerializer(serializers.Serializer):
    ...
    status = serializers.ChoiceField(
        ...
        help_text="New status for the account"
    )
...
class SavingsAccountSummarySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'account_number', 'customer_name', 'current_balance',
            'available_balance', 'currency', 'balance_display', 'status',
            'opening_date', 'last_movement_date', 'total_transactions',
            'recent_transactions', 'average_monthly_balance'
        ]
    ...
    def get_recent_transactions(self, obj):
        ...
        return (
            obj.outgoing_transactions.filter(transaction_date__gte=thirty_days_ago).count() +
            obj.incoming_transactions.filter(transaction_date__gte=thirty_days_ago).count()
        )
